...
def result_sila(fi, f1, f2):
    return math.cos(fi * math.pi / 180) * f1 + math.sqrt(f2 ** 2 - (math.sin(fi * math.pi / 180) * f1) ** 2)



def sila_podyoma(fi, f):
    return f * math.sin((fi - 90) * math.pi / 180)
...
while h < 16000:
    ...
    mo -= Ra0 * t + Ra1 * t
fi1 = 35    # до какого градуса вращать до отброса 1 ступени
dfi = fi1 / (t0 - time)
while time <= t0:
    time += t
    p = plotnost_vosduha(M, R, T, p0, g, h)
...
print("скорость на момент сброса 1 ступени: ", V, " Масса на момент сброса 1 ступени: ", mo, " Высота на момент сброса 1 ступени: ", h)
mo -= m0E
...
print("скорость на момент сброса 2 ступени: ", V, " Масса на момент сброса 2 ступени: ", mo, " Высота на момент сброса 2 ступени: ", h)
mo -= m1E
while time <= t1 + t2:
    time += t
    p = plotnost_vosduha(M, R, T, p0, g, h)
...
print("скорость на момент сброса 3 ступени: ", V, " Масса на момент сброса 3 ступени: ", mo, " Высота на момент сброса 3 ступени: ", h)
mo = mM
V = 21000       # до такой скорости корабль разгонится на орбите земли
Rrz = 149600000000     # расстояние от солнца до земли
MM = 1.9891 * (10 ** 30)     # масса солнца
Rrm = 228000000000      # расстояние от солнца до марса
H = 149600000000 + h     # расстояние от солнца до аппарата
time2 = 0       # время полёта
Sx = 0      # пройденный путь
fiS = 106      # угол между движением аппарата и направляющей к солнцу
t2 = 1     # промежуток времени
while H < 228000000000:
    time2 += t2
    Vh = skorost_podyoma(fiS, V)
    H += Vh * t2
    Sx += V * t2
